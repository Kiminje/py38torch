...
plt.show()
"""
#   x_train = (# of elements, parameters)
...
class LogisticeNeuron:
    ...
    def fit(self, x, y, epochs=10):
        ...
        for i in range(epochs):
            ...
            # tmp_w = np.zeros(w_grad)
            # tmp_b = np.zeros(1)


            # x_k = x[0], y_k = y[0]
            for k in indices:
                z_k = self.forpass(x[k])
                a_k = self.activation(z_k)
                err_k = -(y[k] - a_k)
                w_grad_k, b_grad_k = self.backprop(x[k], err_k)
            ...
            # self.w_history.append(tmp_w)
            # self.b_history.append(tmp_b)
    ...
    def score(self, x_t, y_t):
        return np.mean(self.predict(x_t) == y_t)

neuron = LogisticeNeuron()
neuron.fit(x_train_scaled, y_train)
print(neuron.score(x_test_scaled, y_test))
# plt.boxplot(x_train_scaled)
plt.plot(neuron.losses)
# print("shape of history", neuron.w_history.shape)
# plt.plot(neuron.w_history.T, color='blue')
# plt.plot(neuron.w_history[:, 0], color='green')
# plt.plot(neuron.w_history[0], color='red')
plt.xlabel = "epoch"
plt.ylabel = "loss"
plt.show()
