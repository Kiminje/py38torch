                    ...
                    for cell in Cells:
                        for Emb in EmbSpaces:
                            for reg in regs:
                                ...
                                model_lstm.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
                                model_lstm.summary()
                                print("max length :", maxlength, "Vocab size", Dict, "Embedded size", Emb, "Cell", cell)
                                history = model_lstm.fit(x_train_seq, y_train, epochs=10, batch_size=128,
                                ...
                                plt.title(
                                    "<LOSS: {:0.3f}> Dict {} Emb {} Max: {} Cell: {}".format(loss, Dict, Emb, maxlength
                                                                                                , cell))

                                plt.savefig('ch09_loss_D{}E{}M{}C{}.png'.format(Dict, Emb, maxlength, cell))
                                plt.clf()
                                ...
                                plt.grid(b=True, which='minor', color='#999999', linestyle='-', alpha=0.2)
                                plt.title(
                                    "<Accuracy: {:0.3f}>Dict:{} Emb:{} Max:{} Cell:{}".format(accuracy, Dict, Emb, maxlength
                                                                                                , cell))
                                plt.savefig('ch09_acc_D{}E{}M{}C{}.png'.format(Dict, Emb, maxlength, cell))
                                plt.clf()
